...
contract AgentEscrowTest is Test {
    ...
    function _createIntent(
        ...
        token.approve(address(escrow), type(uint256).max);
        intentId = escrow.createIntent(address(token), _amount, _commitHash);
        vm.stopPrank();
    }
    ...
    function _createAndRevealIntent() internal returns (bytes32 intentId) {
        ...
        vm.prank(payer);
        escrow.revealIntent(intentId, provider, address(token), testAmount, testBond, testSalt);
    }
    ...
    */
    ...
    function test_RevealIntent_RevertInvalidHash() public {
        ...
        escrow.revealIntent(intentId, provider, address(token), testAmount + 1, testBond, testSalt);
    }
    ...
    function test_RevealIntent_RevertBondExceedsAmount() public {
        ...
        escrow.revealIntent(intentId, provider, address(token), testAmount, invalidBond, testSalt);
    }

...
interface IInsurancePool {
    ...
    function claims(bytes32 claimId)
        external
        view
        returns (
            address payer,
    ...
    function epochBucket(address token, uint256 epochStart) external view returns (uint128 openingBalance, uint128 paid);

    function dayBucket(address token, uint256 dayStart) external view returns (uint128 openingBalance, uint128 paid);

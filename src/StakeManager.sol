...
contract StakeManager is IStakeManager, ReentrancyGuard {
     ...
     */
    function depositStake(address token, uint256 amount) 
        external 
        nonReentrant 
    {
     ...
     * @dev Protected by ReentrancyGuard
     */
    function withdrawStake(address token, uint256 amount) 
        external 
        nonReentrant 
    {
        if (token == address(0)) revert ZeroAddress();
     ...
     * @inheritdoc IStakeManager
     */
    function totalStake(address provider, address token) 
        external 
        view 
        returns (uint256) 
    {
     ...
     * @inheritdoc IStakeManager
     */
    function lockedStake(address provider, address token) 
        external 
        view 
        returns (uint256) 
    {
     ...
     * @inheritdoc IStakeManager
     */
    function availableStake(address provider, address token) 
        public 
        view 
        returns (uint256) 
    {
     ...
     * @inheritdoc IStakeManager
     */
    function intentLocks(bytes32 intentId) 
        external 
        view 
